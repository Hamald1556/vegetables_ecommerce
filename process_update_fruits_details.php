...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['description']) && isset($_POST['amount']) && isset($_POST['uses']) && isset($_POST['category'])) {
        ...
        if(isset($_GET['id'])) {
            ...
            $sql = "UPDATE fruits SET description='$description', amount='$amount', uses='$uses', category='$category' WHERE id='$fruit_id'";

            if ($conn->query($sql) === TRUE) {
                // Redirect to fruits_success_update.php with success message
                header("Location: fruits_success_update.php?success=1");
                exit();
            } else {
